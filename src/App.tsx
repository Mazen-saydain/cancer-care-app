...
const App: React.FC = () => {
  ...
  // NOTE: View persistence logic removed as requested. 
  // User will always start at 'home' when authenticated.
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser: any) => {
      if (currentUser) {
        ...
        try {
          ...
          // ALWAYS redirect to HOME when authenticated (Professional App Behavior)
          setView('home');
        ...
        setUser(null);
        // If not authenticated, we let the splash timer handle the 'welcome' view
      }
  ...
  useEffect(() => {
    if (view === 'splash') {
      const timer = setTimeout(() => {
        if (!authLoading) {
           if (user) {
             setView('home'); // Ensure Home if user is already loaded
           } else {
             setView('welcome'); // Standard flow for non-auth users
           }
        }
      }, 1500);
  ...
  if (view === 'welcome') {
    return (
      <div className="min-h-[100dvh] flex flex-col items-center justify-center bg-gradient-to-br from-pink-50 to-pink-100 dark:from-slate-900 dark:to-slate-800 p-8 text-center">
  ...
  if (view === 'login') {
    return (
      <div className="min-h-[100dvh] flex items-center justify-center bg-gray-50 dark:bg-slate-900 p-6">
  ...
  if (view === 'register') {
      return (
        <div className="min-h-[100dvh] flex items-center justify-center bg-gray-50 dark:bg-slate-900 p-6 py-10">
  ...
  const NavItem: React.FC<{ icon: any, label: string, viewName: ViewState }> = ({ icon: Icon, label, viewName }) => (
    <button
      ...
      className={`flex items-center gap-4 w-full px-5 py-4 rounded-xl transition-all text-sm font-medium touch-manipulation ${
        view === viewName 
  ...
  return (
    <div className="min-h-[100dvh] bg-gray-50 dark:bg-slate-900 flex transition-colors duration-300 font-sans overflow-hidden">
      {/* Sidebar - Desktop */}
      ...
      <main className="flex-1 flex flex-col min-w-0 h-[100dvh] overflow-hidden relative bg-gray-50/50 dark:bg-slate-900">
        ...
        {mobileMenuOpen && (
            <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm md:hidden" onClick={() => setMobileMenuOpen(false)}>
                <div className="absolute left-0 top-0 h-[100dvh] w-80 bg-white dark:bg-slate-800 shadow-2xl overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-5 md:p-10 scrollbar-hide">
            <div className="max-w-6xl mx-auto w-full pb-20">
                ...
                {['blood-analysis', 'kidney-analysis', 'liver-analysis', 'diabetes'].includes(view) && (
                 ...
                 {view === 'chat' && (
                    <div className="h-[calc(100dvh-140px)] md:h-[calc(100vh-120px)] flex flex-col animate-fade-in">
